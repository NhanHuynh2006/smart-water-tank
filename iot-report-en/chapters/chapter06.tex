...
The transition between \texttt{IDLE} and \texttt{FILLING} implements equation \eqref{eq:hysteresis} with two separated thresholds plus two timing constraints. The low threshold of 30 percent and the high threshold of 70 percent come from the parameter sweep of Section 8.4, and additionally leave a 15 percent margin up to the 85 percent safety limit to absorb overshoot. The minimum on time of 3 seconds protects the pump from repeated start cycles, the minimum off time of 20 seconds lets the supply voltage settle, and the maximum run time of 200 seconds, roughly twice the measured fill duration, serves as an abnormality threshold.

The minimum on time deserves a note, because the first draft set it to 10 seconds and that value was unsafe. The working volume of this tank is 1 litre, and at the measured 0.36 litres per minute a forced 10 second run adds 0.06 litres, 6 percent of the tank. Holding the pump on for that long while the state machine has no authority to stop it is a guarantee of overshoot past the high threshold. Three seconds is long enough to stop the relay chattering and short enough that the band still governs.
...
\hline
\texttt{OVERFLOW} & Level above 85 percent, raw echo closer than 4.5 cm, or upper float engaged & Stop pump, lock \\
\hline
...
\hline
\texttt{FILL\_TIMEOUT} & Pump running over 200 seconds without reaching the high threshold & Stop pump, lock \\
\hline
\end{tabular}
\end{table}
...
The assignment requires that manual mode must not override safety constraints. The group meets this with an architectural decision inside the firmware: \textbf{every code path capable of starting the pump is forced through one single guard function.} The function returns empty when starting is permitted and a reason code when it is blocked, evaluating six blocking conditions: an unresolved fault, the upper float engaged, a raw echo returning from closer than the safety distance, level past the safety limit, an untrusted level measurement, and the minimum off time not yet elapsed.

An earlier revision listed an empty source tank as a seventh condition. That condition was removed, and the removal is worth recording. The source vessel carries no instrument of any kind, so the firmware has no means of knowing whether it holds water; the condition was in fact reading the lower float of the \emph{storage} tank, which had been mislabelled. Blocking the pump on a quantity the system cannot observe is not a safety measure but a permanent refusal to operate, and dry running is already covered by three rules that rest on quantities the system does observe: absent flow, absent current, and absent progress in the level.
